...
igl::BBWData::BBWData() : partition_unity(false),
                          W0(),
                          active_set_params(),
                          verbosity(0)
{
    ...
    Eigen::PlainObjectBase<DerivedW> &W)
{
  ...
  // std::cout << __FILE__ << " " << __LINE__ << " V=" << V.rows() << " Ele=" << Ele.rows() << ", " << Ele.cols() << std::endl;
  harmonic(V, Ele, 2, Q);
  ...
  if (data.verbosity >= 1)
  {
    cout << "BBW: Computing initial weights for " << m << " handle" << (m != 1 ? "s" : "") << "." << endl;
  }
  ...
  const auto &optimize_weight = [&](const int i)
  {
    ...
    if (data.verbosity >= 1)
    {
      ...
      cout << "BBW: Computing weight for handle " << i + 1 << " out of " << m << "." << endl;
    }
  ...
  if (min_rowsum < 0.1)
  {
    cerr << "bbw.cpp: Warning, minimum row sum is very low. Consider more "
            "active set iterations or enforcing partition of unity."
         << endl;
  }
#endif
