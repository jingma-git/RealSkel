...
// For point handles, find nearest vertexs in the mesh
IGL_INLINE bool igl::boundary_conditions_nearest(
    const Eigen::MatrixXd &V,
    const Eigen::MatrixXi & /*Ele*/,
    const Eigen::MatrixXd &C,
    const Eigen::VectorXi &P,
    ...
    Eigen::MatrixXd &bc)
{
  ...
  // std::cout << "point handles in mesh: " << bci.size() << std::endl;
  ...
  for (int e = 0; e < BE.rows(); e++)
  {
    ...
    for (int i = 0; i < V.rows(); i++)
    {
      ...
      project_to_line(
          ...
          tail(0), tail(1), tail(2),
          t, sqrd);
      // std::cout << "bone" << e << ", tip=" << tip.transpose() << " tail=" << tail.transpose() << " t="
      //           << t << " sqrd=" << sqrd << endl;
      // if (t >= -FLOAT_EPS && t <= (1.0f + FLOAT_EPS) && sqrd <= FLOAT_EPS)
      if (t >= -FLOAT_EPS && t <= (1.0f + FLOAT_EPS) && sqrd <= dis_thresh)
      {
        bci.push_back(i);
        bcj.push_back(P.size() + e);
        bcv.push_back(1.0);
        ...
        //           << " t=" << t << " sqrd=" << sqrd
        //           << " mesh vidx=" << i << std::endl;
      }
    }
  }

  // std::cout << "bone handles in mesh: " << (bci.size() - P.size()) << std::endl;
