    ...
    ArapDOFData<LbsMatrixType, SSCALAR> &data)
{
  ...
  cout << endl
       << "];" << endl
       << "Lapl=sparse(LaplIJV(:,1),LaplIJV(:,2),LaplIJV(:,3)," << Lapl.rows() << "," << Lapl.cols() << ");" << endl;
#endif
  ...
  cout << endl
       << "];" << endl
       << "G_sum=sparse(G_sumIJV(:,1),G_sumIJV(:,2),G_sumIJV(:,3)," << G_sum.rows() << "," << G_sum.cols() << ");" << endl;
#endif
  ...
  cout << endl
       << "];" << endl
       << "CSM=sparse(CSMIJV(:,1),CSMIJV(:,2),CSMIJV(:,3)," << CSM.rows() << "," << CSM.cols() << ");" << endl;
#endif
    ...
    for (int r = 0; r < numRows; r++)
    {
      for (int coord = 0; coord < dim + 1; coord++)
      {
        for (int b = 0; b < numBones; b++)
        {
          ...
          // std::cout << __FILE__ << " " << __LINE__ << " row" << r << " coord" << coord << " bone" << b << " block=\n"
          //           << blok << std::endl;
    ...
    ArapDOFData<LbsMatrixType, SSCALAR> &data)
{
  ...
  // to be there :)
  data.M_FullSolve << (-0.5 * M_Solve.block(0, 0, fsRows, fsRows) * data.M_KG).template cast<SSCALAR>(),
  ...
  for (int i = 0; i < data.dim; i++)
    CSM_M_SSCALAR[i] = data.CSM_M[i].template cast<SSCALAR>();
