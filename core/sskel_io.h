...
inline void writeSSkel(const std::string fname, const SSkel<SSNode> &sskel)
{
    ...
    for (int i = 0; i < num_nodes; ++i)
    {
        ...
        fprintf(fptr, "%d %lg %lg %lg %lg\n", node->idx, pos[0], pos[1], pos[2], node->radius);
    }
...
inline void readSSkel(const std::string fname, SSkel<SSNode> &sskel)
{
    typedef SSNode NodeT;
    typedef SSEdge<NodeT> EdgeT;
